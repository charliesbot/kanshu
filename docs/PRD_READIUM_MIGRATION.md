...
Preferences (fonts, sizes, line height, margins) are updated at runtime by executing lightweight JavaScript to change CSS variables on the document root (`:root`).
...
- The shell script reads `scrollX`, `scrollWidth`, and viewport width internally, computes `pageIndex` and `progressInSpine` with the progress math below, and reports only the derived values to Kotlin after `load`, `hashchange`, programmatic `scrollTo`, and debounced `scroll` events.
