...
internal suspend fun Publication.readFirstReadableChapter(): ReaderDocument? {
  ...
  for (index in readingOrder.indices) {
    ...
    val preview = if (flattened.length <= 80) flattened else flattened.take(80) + "…"
    Log.d(
      TAG,
      "spine[$index]: parsed blocks=${document.blocks.size} chars=$textLength language=${document.language} preview=\"$preview\"",
    )
...
private fun ReaderDocument.flattenedText(): String =
  blocks
    ...
    .filterIsInstance<TextLeaf>()
    .joinToString("") { leaf -> leaf.text }
    .trim()
